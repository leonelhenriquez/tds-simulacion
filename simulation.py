...
simulation.py  -  v3
=====================
Simulación de tráfico utilizando MapGenerator como motor de mapas procedimental.
"""
...
manual_routes = ManualExtraRouteSystem(gen)
...
def _simulation_stats():
    ...
    return {
        ...
        'extra_routes': len(manual_routes),
    ...
    while True:
        ...
        for event in pygame.event.get():
            ...
            if panel_action:
                ...
                elif action == 'drop_route':
                    map_position = _screen_to_map(panel_action['pos'], map_rect)
                    if map_position is not None:
                        manual_routes.place_near(map_position)
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if map_position is not None:
                    if not traffic_accidents.remove_near(map_position):
                        if not manual_routes.remove_near(map_position):
        ...
        if is_playing:
            ...
            with vehicle_lock:
                active_vehicles = list(simulation)
            _, victims = traffic_accidents.generate_from_crossing_conflict(active_vehicles)
            for vehicle in victims:
                _remove_vehicle(vehicle)
        if panel.drag_payload == 'signal':
            traffic_signals.draw_targets(map_surf)
        manual_routes.draw(map_surf)
